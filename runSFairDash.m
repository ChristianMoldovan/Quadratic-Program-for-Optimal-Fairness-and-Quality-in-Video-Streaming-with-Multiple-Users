...
function [stat,result,result2,model,model2]=runSFairDashm(runID,objective,numberOfUsers,userdelay,traffictrace,params,n,bundle,content,segmentsizes,run_length,sumofstalling,segmentlength,alpha,beta,gamma)
...
first.A = sparse(n*numberOfUsers,n*R*numberOfUsers+numberOfUsers);
for i=1:n
    for u=1:numberOfUsers
...
second.A = sparse(n*numberOfUsers,n*R*numberOfUsers+numberOfUsers);
...
% similar average quality for all sessions
third.A = sparse(2*numberOfUsers,n*R*numberOfUsers+numberOfUsers);
for u=1:numberOfUsers
    for j=1:R
        for i = 1:n
            for u_schlange = 1:numberOfUsers
                % mean quality over all users at segment i
%                 third.A(u,(u_schlange-1)*n*R + (i-1)*R + j) = j/numberOfUsers/n;
                if u_schlange ~= u
                    third.A(u,(u_schlange-1)*n*R + (i-1)*R + j) = j;
                end
            end
            % quality of user u at segment i
%             third.A(u,(u-1)*n*R + (i-1)*R + j) = -j/n;
            third.A(u,(u-1)*n*R + (i-1)*R + j) = (numberOfUsers-1) * -j;
        end
        % fairness
        third.A(u,n*R*numberOfUsers + u) = 1;
    end
end
...
for u=1:numberOfUsers
    for j=1:R
        for i = 1:n
            for u_schlange = 1:numberOfUsers
                % mean quality over all users at segment i
%                 third.A(numberOfUsers+u,(u_schlange-1)*n*R + (i-1)*R + j) = -j/numberOfUsers/n;
                if u_schlange ~= u
                    third.A(numberOfUsers+u,(u_schlange-1)*n*R + (i-1)*R + j) = -j;
                end
            end
            % quality of user u at segment i
%             third.A(numberOfUsers+u,(u-1)*n*R + (i-1)*R + j) = j/n;
            third.A(numberOfUsers+u,(u-1)*n*R + (i-1)*R + j) = (numberOfUsers-1) * j;
        end
        % fairness
        third.A(numberOfUsers+u,n*R*numberOfUsers + u) = 1;
    end
end
third.rhs = zeros(2*numberOfUsers,1);
third.sense = repmat('>',1,2*numberOfUsers);
%% Model
model.varnames = names(:)';
model.varnames(end+1:end+numberOfUsers)={'fairness'};
...
%         beta = 0.8;
        ...
        LayerVal=repmat(((1:R)-1)./(R-1)./n./numberOfUsers,n*numberOfUsers,1)'; % mean layer
        fairness = -1.* ones(numberOfUsers,1)./(R-1)./(numberOfUsers-1)./n./numberOfUsers;
        model.obj = [alpha.*LayerVal(:);gamma.*fairness];
        model.obj(isnan(model.obj))=0;
    case CODEC_BW
...
% minimize switches considering the amplitude; 
model.Q = sparse(R*n*numberOfUsers+numberOfUsers,R*n*numberOfUsers+numberOfUsers);
for u = 1:numberOfUsers
    for i=1:n-1
        for j=2:R
                xij = (u-1)*n*R +(i-1)*R+j;
        ...
        % positive half amplitude for upswitches
%         for j=1:R-1
%                 xij = (u-1)*n*R +(i-1)*R+j;
%             for k=j:R
%                 xi1k = (u-1)*n*R +i*R+k;
%                 model.Q(xij,xi1k)=0.25*(k-j);
%                 model.Q(xi1k,xij)=0.25*(k-j);
%             end
%         end
    end
end
...
stat.fairnessQualityStep2=1/(1+cx.^2);
