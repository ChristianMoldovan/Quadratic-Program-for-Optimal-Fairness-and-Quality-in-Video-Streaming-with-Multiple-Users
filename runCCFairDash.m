...
function [stat,result,result2,model,model2]=runCCFairDashm(runID,objective,numberOfUsers,userdelay,traffictrace,params,n,bundle,content,segmentsizes,run_length,sumofstalling,segmentlength,alpha,beta,gamma)
...
% TODO: allow for Fu to be an integer.
...
%                 i_schlange = max(0,min(n,i-1 + userdelay(u_schlange) - userdelay(u)));
                i_schlange = i-1 + userdelay(u_schlange) - userdelay(u);
                if i_schlange < n && i_schlange > 0 && u_schlange ~= u
                    third.A((u-1)*n + i,(u_schlange-1)*n*R + i_schlange*R + j) = j;
                end
            end
            % quality of user u at segment i
            third.A((u-1)*n + i,(u-1)*n*R + (i-1)*R + j) = (numberOfUsers-1) * -j;
        end
        % fairness
    ...
    for u=1:numberOfUsers
        for j=1:R
            for u_schlange = 1:numberOfUsers
                ...
                i_schlange = i-1 + userdelay(u_schlange) - userdelay(u);
                if i_schlange < n && i_schlange > 0 && u_schlange ~= u
                    third.A(n*numberOfUsers+(u-1)*n + i,(u_schlange-1)*n*R + i_schlange*R + j) = -j;
                end
            end
            % quality of user u at segment i
            third.A(n*numberOfUsers+(u-1)*n + i,(u-1)*n*R + (i-1)*R + j) = (numberOfUsers-1) * j;
        end
        % fairness
...
%         beta = 0.8;
        ...
        fairness = -1.* ones(n*numberOfUsers,1)./(R-1)./n./numberOfUsers./(numberOfUsers);
